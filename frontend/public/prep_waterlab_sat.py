#!/usr/bin/env python3
"""Fetch Esri satellite imagery for the Sirajganj bbox, crop to bbox, save as a
single texture (sirajganj_sat.jpg) draped on the Water Lab terrain."""
...
x0f,y0f = deg2num(LAT1,LNG0,Z); x1f,y1f = deg2num(LAT0,LNG1,Z)
xt0,xt1 = int(x0f),int(x1f); yt0,yt1 = int(y0f),int(y1f)
print(f"sat tiles x {xt0}..{xt1} y {yt0}..{yt1} ({(xt1-xt0+1)*(yt1-yt0+1)} tiles)")
TS=256; W=(xt1-xt0+1)*TS; H=(yt1-yt0+1)*TS
    ...
    for tx in range(xt0,xt1+1):
        ...
        for a in range(4):
            try:
                ...
                mosaic.paste(im,((tx-xt0)*TS,(ty-yt0)*TS)); break
            except Exception as e:
                print("  retry",tx,ty,e)
        else: raise SystemExit("sat tile fail")
    print(f"  row {ty} done")
...
crop.save("sirajganj_sat.jpg", quality=88)
print(f"wrote sirajganj_sat.jpg {crop.size}")
